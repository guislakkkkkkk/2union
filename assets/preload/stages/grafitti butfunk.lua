local characterThatSings = dadName
...
function onCreate()
	...
	makeAnimatedLuaSprite('bgbop','grafitti/bgbop jotan',218,345)
	...
	addAnimationByPrefix('bgbop','danceleft','danceleft',24,false)
	addAnimationByPrefix('bgbop','danceright','danceright',24,false)
...
function onBeatHit()
	if partytime then
		if curBeat % 2 == 0 then
			playAnim('bgbop','danceleft',true,false,0)
		else
			playAnim('bgbop','danceright',true,false,0)
		end
	else
		if curBeat % 2 == 0 then
			playAnim('bgbop',dadName,true,false,0)
